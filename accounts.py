...
class Account(BaseModel):
    ...
    def get_profit_loss(self, portfolio_value: float):
...
if __name__ == "__main__":
    account = Account(name="johndoe", balance=10000, strategy="", holdings={}, transactions=[], portfolio_value_time_series=[])
    account.deposit(1000)
    account.buy_shares("AAPL", 5, "Long term investment")
    account.sell_shares("AAPL", 2, "Taking profit")
    print(f"Current Holdings: {account.get_holdings()}")
    print(f"Total Portfolio Value: {account.calculate_portfolio_value()}")
    print(f"Profit/Loss: {account.get_profit_loss(account.calculate_portfolio_value())}")
